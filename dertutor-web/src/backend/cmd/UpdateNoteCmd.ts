...
export class UpdateNoteCmd implements Runnable {
  ...
  private async sending(op: RXOperation<any, RestApiError>) {
    ...
    const [response, body] = await this.api.sendRequest('PUT', '/notes/' + this.note.id, JSON.stringify(this.note.serialize()))
    if (response?.ok) {
      op.success(body)
    } else {
